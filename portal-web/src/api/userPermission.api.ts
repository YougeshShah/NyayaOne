...
export const userPermissionApi = {
  async listForUser(userId: string): Promise<UserPermissionsResult> {
    const { data } = await apiClient.get<ApiSuccessResponse<UserPermissionsResult>>(`/user-permissions/${userId}`);
    return data.data;
  },

  async setOverride(userId: string, permissionId: string, granted: boolean, reason?: string) {
    const { data } = await apiClient.put<ApiSuccessResponse<any>>(`/user-permissions/${userId}`, {
      permissionId,
      granted,
      reason,
    });
  ...
  async removeOverride(userId: string, permissionId: string) {
    await apiClient.delete(`/user-permissions/${userId}/${permissionId}`);
  },
};
