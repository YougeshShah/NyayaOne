...
// Gemini-powered /chatbot/message endpoint used by the mobile apps.
    ...
    <Box sx={{ position: "fixed", bottom: 24, right: 24, zIndex: 1300 }}>
      <Collapse in={open}>
        <Paper elevation={6} sx={{ width: 340, height: 460, display: "flex", flexDirection: "column", mb: 2, borderRadius: 3, overflow: "hidden" }}>
          <Box sx={{ bgcolor: "#0F172A", color: "#fff", px: 2, py: 1.5, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, bgcolor: "#F3F4F6", px: 1.5, py: 0.75 }}>
            ...
            <Typography fontSize={10.5} color="text.secondary">
              Don't share case-sensitive details — messages go to a third-party AI service.
            </Typography>
          </Box>
          <Box ref={scrollRef} sx={{ flex: 1, overflowY: "auto", p: 1.5 }}>
            {messages.length === 0 && (
              <Typography variant="caption" color="text.secondary" sx={{ display: "block", textAlign: "center", mt: 4 }}>
                Ask general questions about using the app or platform features.
              </Typography>
            )}
            {messages.map((m, i) => (
              <Box key={i} sx={{ mb: 1.5, display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                <Box
                  sx={{
                    maxWidth: "85%",
                    bgcolor: m.role === "user" ? "#0F172A" : m.isError ? "#FEE2E2" : "#F3F4F6",
      ...
      {!open && (
        <Fab onClick={() => setOpen(true)} sx={{ bgcolor: "#0F172A", color: "#fff", "&:hover": { bgcolor: "#1e293b" } }}>
