...
export function VerifyEmailPage() {
  ...
  const verify = useMutation({
    mutationFn: () => apiClient.post("/email-verification/verify-email", { email, code }),
    onSuccess: () => {
      setSuccess(true);
  ...
  const resend = useMutation({
    mutationFn: () => apiClient.post("/email-verification/send-code", { email, purpose: "REGISTRATION" }),
  });
    ...
    <Box sx={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", bgcolor: "#F9FAFB", p: 2 }}>
      <Paper elevation={0} sx={{ p: 4, width: 420, borderRadius: 3, border: "1px solid #E5E7EB" }}>
        ...
        <Typography variant="body2" color="text.secondary" textAlign="center" sx={{ mb: 3 }}>
          Once your organization is approved, we send a 6-digit code to your admin email. Enter it below to activate your account.
        </Typography>
        ...
        {success ? (
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <TextField label="Admin Email" type="email" required fullWidth value={email} onChange={(e) => setEmail(e.target.value)} />
            <TextField
              label="6-Digit Code"
              required
              fullWidth
